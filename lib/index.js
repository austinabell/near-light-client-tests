...
function computeOutcomeRoot(outcomeWithId, outcomeRootProof) {
    ...
    const borshStatus = (status) => {
        if (status === provider_1.ExecutionStatusBasic.Pending) {
            ...
            return new BorshPartialExecutionStatus({
                successValue: bs58_1.default.decode(status.SuccessValue),
            });
        }
    ...
    const partialExecOutcome = {
        receiptIds: receiptIds,
        gasBurnt: outcomeWithId.outcome.gas_burnt,
        // TODO missing declarations of object types in NAJ
        tokensBurnt: outcomeWithId.outcome.tokens_burnt,
        executorId: outcomeWithId.outcome.executor_id,
        status: borshStatus(outcomeWithId.outcome.status),
    };
    const serializedOutcome = (0, serialize_1.serialize)(SCHEMA, partialExecOutcome);
    const logsHashes = outcomeWithId.outcome.logs.map((log) => {
        return crypto_1.default.createHash("sha256").update(bs58_1.default.decode(log)).digest();
    });
    const outcomeHashes = new Array(bs58_1.default.decode(outcomeWithId.id), serializedOutcome, ...logsHashes);
...
function validateExecutionProof(proof) {
    ...
    if (!blockOutcomeRoot.equals(bs58_1.default.decode(proof.block_header_lite.inner_lite.outcome_root))) {
    ...
    if (!blockMerkleRoot.equals(bs58_1.default.decode(proof.block_header_lite.inner_lite.block_merkle_root))) {
