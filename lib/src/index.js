...
currentBlockProducers, newBlock) {
    ...
    if (newBlock.approvals_after_next.length !== blockProducers.length) {
        throw new Error("Number of approvals from the next block must match the number of block producers in the current epoch");
    }
    // (4) and (5)
    let totalStake = 0;
    let approvedStake = 0;
        ...
        const borshBps = bps.map((bp) => {
            ...
            return new BorshValidatorStakeView({
                v1: new BorshValidatorStakeViewV1({
                    ...
                    public_key: bs58_1.default.decode(bp.public_key),
                    stake: bp.stake,
                }),
            });
        });
        const serializedBps = (0, serialize_1.serialize)(SCHEMA, borshBps);
